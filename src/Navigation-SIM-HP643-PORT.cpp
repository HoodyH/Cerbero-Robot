...
void N_FotoresistorClass::cerca() {

	Fotoresistor.letturaDX(valueDX); //carica il valore letto tramite la funzione su una Variabile globale della classe Fotoresitor
	Fotoresistor.letturaSX(valueSX);
	Fotoresistor.letturaC(valueC);

	//controlla se la fotoresistenza rileva luce con intensit� nel range impostato
	if (valueDX >= LuxMIN && valueDX <= LuxMAX) {
		vFotoDX = 1;
		valuePWM = valueDX;
		valuePWM = map(valuePWM, LuxMIN, LuxMAX, 5, 250);
		analogWrite(ledFotoDX, valuePWM);
	}//end if valueDX>=LuxMIN && valueDX<=LuxMAX

	else {
		vFotoDX = 0;
		analogWrite(ledFotoDX, 0);
	}//end else valueDX>=LuxMIN && valueDX<=LuxMAX

	 //verifica se il valore della fotoresistenza in questione � calato
	if (valueDX>valueDXprecedente) {
		dFotoDX = 1;
	}//end if valueDX>valueDXprecedente

	else {
		dFotoDX = 0;
	}//end else valueDX>valueDXprecedente
	valueDXprecedente = valueDX;

	//------------------------------------

	//controlla se la fotoresistenza rileva luce con intensit� nel range impostato
	if (valueSX >= LuxMIN && valueSX <= LuxMAX) {
		vFotoSX = 1;
		valuePWM = valueSX;
		valuePWM = map(valuePWM, LuxMIN, LuxMAX, 5, 250);
		analogWrite(ledFotoSX, valuePWM);
	}//end if valueSX>=LuxMIN && valueDX<=LuxMAX

	else {
		vFotoSX = 0;
		analogWrite(ledFotoSX, 0);
	}

	//verifica se il valore della fotoresistenza in questione � calato
	if (valueSX>valueSXprecedente) {
		dFotoSX = 1;
	}//end if valueSX>valueSXprecedente

	else {
		dFotoSX = 0;
	}//end else valueSX>valueSXprecedente
	valueSXprecedente = valueSX;

	//------------------------------------

	//controlla se la fotoresistenza rileva luce con intensit� nel range impostato
	if (valueC >= LuxMIN && valueDX <= LuxMAX) {
		vFotoC = 1;
		valuePWM = valueC;
		valuePWM = map(valuePWM, LuxMIN, LuxMAX, 5, 250);
		analogWrite(ledFotoC, valuePWM);
	}//end if valueC>=LuxMIN && valueDX<=LuxMAX
	else {
		vFotoC = 0;
		analogWrite(ledFotoC, 0);
	}

	//verifica se il valore della fotoresistenza in questione � calato
	if (valueC>valueCprecedente) {
		dFotoC = 1;
	}//end if valueC>valueCprecedente
	else {
		dFotoC = 0;
	}//end else valueC>valueCprecedente
	valueCprecedente = valueC;

	//------------------------------------

	deltaFotoDxSx = valueDX - valueSX;

	//------------------------------------

	int lettura = (vFotoSX << 2) | (vFotoC << 1) | vFotoDX; //converte i valori dei 4 sensori in un unico numero binario inserendo i bit nelle posizioni assegnate

															//controllo dei casi in cui sono attivi i vari sensori controllati ciclicamente e assegnazione di un nome tramite la funzione enum

															//      vFotoSX,vFotoC,vFotoDX  (valore dei bit secondo le posizioni)

	if (lettura == 0b0000) fonteLuce = vuoto;           // (nessuna luce rilevata)
	if (lettura == 0b0001) fonteLuce = gradi40;         // (luce destra)
	if (lettura == 0b0010) fonteLuce = gradi90;         // (luce centro)
	if (lettura == 0b0011) fonteLuce = gradi65;         // (vFotoC,vFotoDX)

	if (lettura == 0b0100) fonteLuce = gradi140;        // (luce sinistra)
	if (lettura == 0b0101) fonteLuce = fSXfDX;          // (vFotoSX,vFotoDX)
	if (lettura == 0b0110) fonteLuce = gradi115;        // (vFotoSX,vFotoC)
	if (lettura == 0b0111) fonteLuce = casoALL;         // (vFotoSX,vFotoC,vFotoDX)
  ...
  Bluetooth.stato=4; //find lights
	...
	case gradi40:
		...
		while (vFotoC == 0) {
			...
			Motore.right(velocitaMaxLD, TEMPOINCREMENTO, 0, StepAccelerazioneRotate, velMin3);
			Navigation.updateSensors();
      ...
      if (vFotoSX == 1) {
           break;
			...
			if (currentMillis - previousMillis >= tGuardiaFoto1) {  //conrolla se � passato il tempo di guardia prestabilito
        
         delta = eqLD;
			   Motore.straight(velocitaMaxLD, TEMPOINCREMENTO, delta, noSpeedReduction, StepAccelerazioneStraight, velMin3);
				 if (vFotoSX == 1) {
				    break;
				 }//end if vFotoSX == 0

				 if (currentMillis - previousMillis >= tGuardiaFoto2) {  //conrolla se � passato il tempo di guardia prestabilito
					 if (vFotoDX == 0) {
					  	break;
					    }//end if vFotoDX == 0 
				 }//end if currentMillis - previousMillis >= tGuardiaFoto2
         
			}//end if currentMillis - previousMillis >= tGuardiaFoto1
		}//end while vFotoC==0
	...
	case gradi90:
		//Serial.println("luceGradi90"); 

		while (Navigation.vRxDX == 0 && Navigation.vRxSX == 0) {

			if (dFotoC == 1) {
				if (N_fotoresistor.deltaFotoDxSx>0) {
					delta = eqLLD;
					Motore.straight(velocitaMaxLLD, TEMPOINCREMENTO, delta, noSpeedReduction, StepAccelerazioneStraight, velMin3);
				}//end if deltaDxSX

				else {
					delta = eqLLS;
					Motore.straight(velocitaMaxLLS, TEMPOINCREMENTO, delta, noSpeedReduction, StepAccelerazioneStraight, velMin3);
				}//end else
			}//end if dFotoC==1

			Navigation.updateSensors();

		}//end while vRxDX==0 && vRxDX==0 

		for (int i = 0; i < tAvvicinamentoStop; i++)
		{
			Motore.straight(100, TEMPOINCREMENTO, 0, noSpeedReduction, StepAccelerazioneStraight, velMin3);
			delay(1);
		}//end for
		trovata();
	...
	case gradi140:
		//Serial.println("luceGradi140");

		currentMillis = millis();
		previousMillis = currentMillis;
		while (vFotoC == 0) {
			Motore.left(velocitaMaxLS, TEMPOINCREMENTO, 0, StepAccelerazioneRotate, velMin3);
			Navigation.updateSensors();
      ...
      if (vFotoDX == 1) {
           break;
			...
			if (currentMillis - previousMillis >= tGuardiaFoto1) {  //conrolla se � passato il tempo di guardia prestabilito
				...
				delta = eqLS;
				Motore.straight(velocitaMaxLS, TEMPOINCREMENTO, delta, noSpeedReduction, StepAccelerazioneStraight, velMin3);
	...
	case casoALL:
		//Serial.println("luceCasoALL");

		while (Navigation.vRxDX == 0 && Navigation.vRxSX == 0) {
			if (dFotoC == 1) {
				if (Navigation.deltaPingDxSx>0) {
					for (int i = 0; i < 80; i++)
					{
						delta = eqLLD;
						Motore.straight(velocitaMaxLLD, TEMPOINCREMENTO, delta, noSpeedReduction, StepAccelerazioneStraight, velMin3);
...
void N_FotoresistorClass::trovata() {

	Motore.wait(1);
  Bluetooth.stato=7; //find lights
  Navigation.updateSensors();
